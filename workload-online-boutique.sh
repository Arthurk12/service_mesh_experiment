...
INDEX_URL=${GATEWAY}
PRODUCTS_URL=${GATEWAY}/product/
PRODUCTS=(OLJCESPC7Z 66VCHSJNUP 1YMWWN1N4O L9ECAV7KIM 2ZYFJ3GM2N 0PUK6V6EV0 LS4PSXUNUM 9SIQT8TOJO 6E92ZMYYFZ)
VIEW_CART_URL=${GATEWAY}/cart

INDEX_URL_OUTPUT="${EXPERIMENT_ID}_index"
PRODUCTS_URL_OUTPUT="${EXPERIMENT_ID}_products"
VIEW_CART_URL_OUTPUT="${EXPERIMENT_ID}_cart"
...
echo "time_namelookup (seconds),time_connect (seconds),time_appconnect (seconds),time_pretransfer (seconds),time_redirect (seconds),time_starttransfer (seconds),time_total (seconds)" > ${PRODUCTS_URL_OUTPUT}.csv
echo "time_namelookup (seconds),time_connect (seconds),time_appconnect (seconds),time_pretransfer (seconds),time_redirect (seconds),time_starttransfer (seconds),time_total (seconds)" > ${VIEW_CART_URL_OUTPUT}.csv


for ((i=1; i<=${TIMES}; i++)); do
	echo "Requesting product page"
	make_curl_request_and_save_times $INDEX_URL $INDEX_URL_OUTPUT
	for product_id in "${PRODUCTS[@]}"; do
		product_url="${PRODUCTS_URL}${product_id}"
		echo "Requesting product page: id ${product_id}   url ${product_url}"
		make_curl_request_and_save_times $product_url $PRODUCTS_URL_OUTPUT
	done
	echo "Requesting cart page"
	make_curl_request_and_save_times $VIEW_CART_URL $VIEW_CART_URL_OUTPUT
done
